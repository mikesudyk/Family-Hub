...
router.post('/', async (req, res) => {
  const { familyId } = req;
  const { date, title, time, color = 'gray', icon = '📅', notes, url, location } = req.body;
  if (!date || !title) return res.status(400).json({ error: 'date and title required' });
  try {
    const r = await pool.query(
      'INSERT INTO calendar_events (family_id, date, title, time, color, icon, notes, url, location) VALUES ($1,$2,$3,$4,$5,$6,$7,$8,$9) RETURNING *',
      [familyId, date, title, time, color, icon, notes || null, url || null, location || null]
    );
    const e = r.rows[0];
    const out = { id: e.id, date: e.date.toISOString().split('T')[0], title: e.title, time: e.time, endTime: e.end_time || null, color: e.color, icon: e.icon, notes: e.notes || null, url: e.url || null, location: e.location || null };
...
router.put('/:id', async (req, res) => {
  const { familyId } = req;
  const { title, date, time, notes, url, location } = req.body;
  try {
    const r = await pool.query(
      ...
      [title, date, time, notes ?? null, url ?? null, location ?? null, req.params.id, familyId]
    );
    const e = r.rows[0];
    const out = { id: e.id, date: e.date.toISOString().split('T')[0], title: e.title, time: e.time, endTime: e.end_time || null, color: e.color, icon: e.icon, notes: e.notes || null, url: e.url || null, location: e.location || null, external_id: e.external_id, provider: e.provider };
